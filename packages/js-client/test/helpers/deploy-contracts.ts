...
export async function deploy(): Promise<Deployment> {
  ...
  const ens = await deployEnsContracts(
    deployOwnerWallet,
  );
...
export async function deploySimpleStorageContracts(
  ...
  const simpleStorageRepo = aragonContracts.PluginRepo__factory
    .connect(simpleStorageRepoAddress, signer);
...
export async function deployOsxContracts(
  ...
  try {
    ...
    const daoRegistrarInstance = aragonContracts.ENSSubdomainRegistrar__factory
      .connect(daoRegsitrarProxy.address, signer);
    const pluginRegistrarInstance = aragonContracts
      .ENSSubdomainRegistrar__factory
      .connect(pluginRegistrarProxy.address, signer);
    ...
    const daoFactory = await daoFactoryfactory.connect(signer)
      .deploy(
