...
export async function buildSimpleStorageDao(
  deployment: Deployment,
) {
  try {
    const latestVersion = await deployment.simpleStorageRepo
      ["getLatestVersion(address)"](deployment.simpleStorageSetup.address);
    return await createDao(
      ...
      [
        {
          pluginSetupRef: {
            pluginSetupRepo: deployment.simpleStorageRepo.address,
